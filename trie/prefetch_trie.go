...
func NewPrefetch(root common.Hash, db *Database) (*PrefetchTrie, error) {
	...
	pt := PrefetchTrie{
		trie:     *trie,
		jobs:     make(chan prefetchJobWrapper, 32),
		rootLock: &sync.RWMutex{},
	}
...
// Start starts the PrefetchTrie loop
func (t *PrefetchTrie) Start() {
	go t.loop()
}

// Sync blocks until all prefetch jobs for a PrefetchTries
// have completed
func (t *PrefetchTrie) Sync() {
...
// Kill Syncs then kills the inner loop
func (t *PrefetchTrie) Kill() {
	t.Sync()
	prefetchSuccess := make(chan error, 1)
	t.jobs <- prefetchJobWrapper{
		trieKillJob{},
...
// TryGet returns TryGet from the SecureTrie
func (t *PrefetchTrie) TryGet(key []byte) ([]byte, error) {
