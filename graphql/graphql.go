...
func (b *Block) Call(ctx context.Context, args struct {
	...
	result, err := ethapi.DoCall(ctx, b.backend, args.Data, *b.numberOrHash, nil, 5*time.Second, b.backend.RPCGasCap(), nil)
	if err != nil {
		return nil, err
	}
	status := Long(1)
...
func (p *Pending) Call(ctx context.Context, args struct {
	...
	pendingBlockNr := rpc.BlockNumberOrHashWithNumber(rpc.PendingBlockNumber)
	result, err := ethapi.DoCall(ctx, p.backend, args.Data, pendingBlockNr, nil, 5*time.Second, p.backend.RPCGasCap(), nil)
	if err != nil {
		return nil, err
	}
	status := Long(1)
