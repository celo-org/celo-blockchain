...
func (rs Receipts) DeriveFields(config *params.ChainConfig, hash common.Hash, number uint64, txs Transactions) error {
	...
	if len(txs) != len(rs) {
		...
		if len(txs)+1 != len(rs) || !isBlockReceipt(rs[len(rs)-1]) {
			return errors.New("transaction and receipt count mismatch")
		}
	}

	// len(rs) is not always strictly equal to len(txs) because of the block finalization receipt (IBFT)
	for i := 0; i < len(txs); i++ {
	...
	// Handle block finalization receipt (only IBFT) which is always the last receipt
	if len(txs)+1 == len(rs) {
		j := len(txs)
...
func isBlockReceipt(receipt *Receipt) bool {
