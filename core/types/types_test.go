...
func benchRLP(b *testing.B, encode bool) {
	...
	signer := NewGingerbreadSigner(big.NewInt(1337))
	feeCurrencyAddr := common.HexToAddress("01")
	gatewayFeeRecipientAddr := common.HexToAddress("02")
		...
		},
		{
			...
			MustSignNewTx(key, signer,
				&CeloDynamicFeeTx{
					...
					GatewayFeeRecipient: &gatewayFeeRecipientAddr,
					GatewayFee:          big.NewInt(500),
				}),
		},
		{
			"celo-dynamic-fee-v2-transaction",
			MustSignNewTx(key, signer,
				&CeloDynamicFeeTxV2{
					Nonce:       1,
					Gas:         1000000,
