...
func (p *StateProcessor) Process(block *types.Block, statedb *state.StateDB, cfg vm.Config) (types.Receipts, []*types.Log, uint64, error) {
	...
	for i, tx := range block.Transactions() {
		...
		receipt, _, err := ApplyTransaction(p.config, p.bc, nil, gp, statedb, header, tx, usedGas, cfg, p.gcWl, p.preAdd)
		if err != nil {
...
func ApplyTransaction(config *params.ChainConfig, bc ChainContext, author *common.Address, gp *GasPool, statedb *state.StateDB, header *types.Header, tx *types.Transaction, usedGas *uint64, cfg vm.Config, gcWl *GasCurrencyWhitelist, preAdd *PredeployedAddresses) (*types.Receipt, uint64, error) {
	...
	context := NewEVMContext(msg, header, bc, author, abeAddress, reserveAddress)
