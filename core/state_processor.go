...
func (p *StateProcessor) Process(block *types.Block, statedb *state.StateDB, cfg vm.Config) (types.Receipts, []*types.Log, uint64, error) {
	...
	for i, tx := range block.Transactions() {
		...
		infraFraction, _ := gpm.GetInfrastructureFraction(statedb, header)
		gasPriceMinimum, _ := gpm.GetGasPriceMinimum(tx.GasCurrency(), statedb, header)
...
func ApplyTransaction(config *params.ChainConfig, bc ChainContext, author *common.Address, gp *GasPool, statedb *state.StateDB, header *types.Header, tx *types.Transaction, usedGas *uint64, cfg vm.Config, gasPriceMinimum *big.Int, infraFraction *gpm.InfrastructureFraction) (*types.Receipt, uint64, error) {
