...
func ReadBlock(db ethdb.Reader, hash common.Hash, number uint64) *types.Block {
	...
	return types.NewBlockWithHeader(header).WithBody(body.Transactions, body.EpochSnarkData)
}
...
func WriteAncientBlocks(db ethdb.AncientWriter, blocks []*types.Block, receipts []types.Receipts, _ *big.Int) (int64, error) {
	var (
	...
	return db.ModifyAncients(func(op ethdb.AncientWriteOp) error {
		for i, block := range blocks {
			...
			header := block.Header()
			if err := writeAncientBlock(op, block, header, stReceipts, big.NewInt(int64(block.NumberU64())+1)); err != nil {
				return err
			}
		}
		return nil
...
func ReadBadBlock(db ethdb.Reader, hash common.Hash) *types.Block {
	...
	for _, bad := range badBlocks {
		if bad.Header.Hash() == hash {
			return types.NewBlockWithHeader(bad.Header).WithBody(bad.Body.Transactions, bad.Body.EpochSnarkData)
		}
	}
	return nil
}
...
func ReadAllBadBlocks(db ethdb.Reader) []*types.Block {
	...
	for _, bad := range badBlocks {
		blocks = append(blocks, types.NewBlockWithHeader(bad.Header).WithBody(bad.Body.Transactions, bad.Body.EpochSnarkData))
	}
	return blocks
}
