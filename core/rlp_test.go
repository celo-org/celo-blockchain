...
func getBlock(transactions int, chainSize, dataSize int) *types.Block {
	var (
	...
	blocks, _ := GenerateChain(params.TestChainConfig, genesis, engine, db, chainSize+1,
		func(n int, b *BlockGen) {
			if n == chainSize {
				// Add transactions and stuff on the last block
				for i := 0; i < transactions; i++ {
					tx, _ := types.SignTx(types.NewTransaction(uint64(i), aa,
						big.NewInt(0), 50000, b.MinimumGasPrice(nil), nil, nil, nil, make([]byte, dataSize)), types.HomesteadSigner{}, key)
					b.AddTx(tx)
				}
			}
		})
...
func TestRlpIterator(t *testing.T) {
	for _, tt := range []struct {
		...
		testRlpIterator(t, tt.txs, tt.chainSize, tt.datasize)
	}
}

func testRlpIterator(t *testing.T, txs, chainSize, datasize int) {
	desc := fmt.Sprintf("%d txs [%d datasize]", txs, datasize)
	bodyRlp, _ := rlp.EncodeToBytes(getBlock(txs, chainSize, datasize).Body())
