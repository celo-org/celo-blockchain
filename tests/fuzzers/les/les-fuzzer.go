...
import (
	...
	l "github.com/celo-org/celo-blockchain/les"
...
func makechain() (bc *core.BlockChain, addrHashes, txHashes []common.Hash) {
	...
	blocks, _ := core.GenerateChain(gspec.Config, genesis, mockEngine.NewFaker(), db, testChainLen,
		func(i int, gen *core.BlockGen) {
			...
			nonce := uint64(i)
			if i%4 == 0 {
				tx, _ = types.SignTx(types.NewContractCreation(nonce, big.NewInt(0), 200000, gen.MinimumGasPrice(nil), nil, nil, nil, testContractCode), signer, bankKey)
