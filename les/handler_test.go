			...
			for {
				...
				if !seen[num] {
					...
					if len(bodies) < tt.expected {
						bodies = append(bodies, &types.Body{Transactions: block.Transactions(), EpochSnarkData: block.EpochSnarkData()})
					}
					break
				}
			}
...
func testTransactionStatus(t *testing.T, protocol int) {
	...
	gchain, _ := core.GenerateChain(params.IstanbulTestChainConfig, chain.GetBlockByNumber(0), mockEngine.NewFaker(), server.db, 1, func(i int, block *core.BlockGen) {
		block.AddTx(tx1)
		block.AddTx(tx2)
	})
