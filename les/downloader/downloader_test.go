...
func (dlp *downloadTesterPeer) RequestBodies(hashes []common.Hash) error {
	txs, randomness := dlp.chain.bodies(hashes)
	go dlp.dl.downloader.DeliverBodies(dlp.id, txs, randomness)
	return nil
}
...
func TestInactiveDownloader64(t *testing.T) {
	...
	if err := tester.downloader.DeliverBodies("bad peer", [][]*types.Transaction{}, []*types.Randomness{}); err != errNoSyncActive {
		t.Errorf("error mismatch: have %v, want %v", err, errNoSyncActive)
	}
