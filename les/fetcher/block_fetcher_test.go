...
var (
	...
	unknownBlock = types.NewBlock(&types.Header{}, nil, nil, trie.NewStackTrie(nil))
)
...
func makeChain(n int, seed byte, parent *types.Block) ([]common.Hash, map[common.Hash]*types.Block) {
	blocks, _ := core.GenerateChain(params.IstanbulTestChainConfig, parent, mockEngine.NewFaker(), testdb, n, func(i int, block *core.BlockGen) {
		block.SetCoinbase(common.Address{seed})

		// If the block number is multiple of 3, send a bonus transaction to the miner
		if parent == genesis && i%3 == 0 {
			signer := types.MakeSigner(params.IstanbulTestChainConfig, block.Number())
			tx, err := types.SignTx(types.NewTransaction(block.TxNonce(testAddress), common.Address{seed}, big.NewInt(1000), params.TxGas, block.MinimumGasPrice(nil), nil), signer, testKey)
			if err != nil {
				panic(err)
			}
			block.AddTx(tx)
...
func (f *fetcherTester) makeBodyFetcher(peer string, blocks map[common.Hash]*types.Block, drift time.Duration) bodyRequesterFn {
	...
	return func(hashes []common.Hash) error {
		...
		// Return on a new thread
		go f.fetcher.FilterBodies(peer, blockHashes, transactions, epochSnarkData, time.Now().Add(drift))
