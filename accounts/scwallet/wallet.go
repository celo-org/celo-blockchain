...
type Wallet struct {
	...
	deriveChain     celo.ChainStateReader     // Blockchain state reader to discover used account with
...
// chain state reader.
func (w *Wallet) SelfDerive(bases []accounts.DerivationPath, chain celo.ChainStateReader) {
	w.lock.Lock()
	defer w.lock.Unlock()
