 ...
 * Instead of using the default data directory (`~/.celo` on Linux for example), Geth will nest
   itself one level deeper into a `testnet` subfolder (`~/.celo/testnet` on Linux). Note, on OSX
