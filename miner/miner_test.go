...
func (bc *testBlockChain) CurrentBlock() *types.Block {
	return types.NewBlock(&types.Header{}, nil, nil, trie.NewStackTrie(nil))
}
...
func TestMiner(t *testing.T) {
	...
	validatorAddress := common.HexToAddress("0x12345")
	miner.Start(validatorAddress, validatorAddress)
...
func TestMinerStartStopAfterDownloaderEvents(t *testing.T) {
	...
	validator2Address := common.HexToAddress("0x678910")
	miner.Start(validator2Address, validator2Address)
	waitForMiningState(t, miner, true)
...
// TestMinerSetEtherbase checks that validator becomes set even if mining isn't
// possible at the moment
func TestMinerSetValidator(t *testing.T) {
	miner, mux := createMiner(t)
	// Start with a 'bad' mining address
	deadAddress := common.HexToAddress("0xdead")
	miner.Start(deadAddress, deadAddress)
	waitForMiningState(t, miner, true)
	// Start the downloader
	mux.Post(downloader.StartEvent{})
	waitForMiningState(t, miner, false)
	// Now user tries to configure proper mining address
	validatorAddress := common.HexToAddress("0x1337")
	miner.Start(validatorAddress, validatorAddress)
	// Stop the downloader and wait for the update loop to run
	mux.Post(downloader.DoneEvent{})
	...
	if got, exp := miner.validator, validatorAddress; got != exp {
		t.Fatalf("Wrong validator, got %x expected %x", got, exp)
	}
}
