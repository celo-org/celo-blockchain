...
type worker struct {
	...
	taskCh             chan *task
	resultCh           chan *istanbul.BlockProcessResult
	startCh            chan struct{}
	exitCh             chan struct{}
...
func newWorker(config *Config, chainConfig *params.ChainConfig, engine consensus.Engine, eth Backend, mux *event.TypeMux, isLocalBlock func(*types.Block) bool, db *ethdb.Database, init bool) *worker {
	worker := &worker{
		...
		resultCh:           make(chan *istanbul.BlockProcessResult, resultQueueSize),
...
func (w *worker) commitNewWork(interrupt *int32, noempty bool, timestamp int64) {
	...
	if w.isRunning() {
		if w.txFeeRecipient == (common.Address{}) {
			log.Error("Refusing to mine without etherbase")
			return
		}
		header.Coinbase = w.txFeeRecipient
	}
	...
	if w.isRunning() && random.IsRunning() {
		...
		lastRandomness, err := random.GetLastRandomness(w.validator, w.db, w.current.header, w.current.state, w.chain, randomSeed)
		if err != nil {
	...
	if len(localTxs) > 0 {
		...
		if w.commitTransactions(txs, w.txFeeRecipient, interrupt) {
			return
		}
	}
	if len(remoteTxs) > 0 {
