...
func odrContractCall(ctx context.Context, db ethdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
	...
	for i := 0; i < 3; i++ {
		...
		msg := callmsg{types.NewMessage(testBankAddress, &testContractAddr, 0, new(big.Int), 1000000, new(big.Int), nil, data, false)}
...
func testChainGen(i int, block *core.BlockGen) {
	...
	case 1:
		...
		nonce++
		tx3, _ := types.SignTx(types.NewContractCreation(nonce, big.NewInt(0), 1000000, big.NewInt(0), nil, testContractCode), signer, acc1Key)
	...
	case 2:
		...
		tx, _ := types.SignTx(types.NewTransaction(block.TxNonce(testBankAddress), testContractAddr, big.NewInt(0), 100000, nil, nil, data), signer, testBankKey)
		block.AddTx(tx)
	case 3:
...
func testChainOdr(t *testing.T, protocol int, fn odrTestFn) {
	...
	lightchain, err := NewLightChain(odr, params.TestChainConfig, ethash.NewFullFaker(), vm.Config{})
	if err != nil {
		t.Fatal(err)
	}
