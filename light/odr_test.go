...
func odrContractCall(ctx context.Context, db ethdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
	...
	for i := 0; i < 3; i++ {
		...
		ret, _, _, _ := core.ApplyMessage(vmenv, msg, gp, nil, gasprice.FallbackGasPriceMinimum, &gasprice.FallbackInfraFraction)
		res = append(res, ret...)
		if st.Error() != nil {
			return res, st.Error()
		}
...
func testChainGen(i int, block *core.BlockGen) {
	...
	case 1:
		...
		nonce++
		tx3, _ := types.SignTx(types.NewContractCreation(nonce, big.NewInt(0), 1000000, big.NewInt(0), nil, nil, testContractCode), signer, acc1Key)
