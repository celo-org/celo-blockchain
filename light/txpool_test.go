...
func TestTxPool(t *testing.T) {
	...
	for i := range testTx {
		testTx[i], _ = types.SignTx(types.NewCeloTransaction(uint64(i), acc1Addr, big.NewInt(10000), params.TxGas, gp, nil, nil, nil, nil), types.HomesteadSigner{}, testBankKey)
	}
	...
	gchain, _ := core.GenerateChain(params.IstanbulTestChainConfig, genesis, mockEngine.NewFaker(), sdb, poolTestBlocks, txPoolTestChainGen)
	...
	lightchain, _ := NewLightChain(odr, params.IstanbulTestChainConfig, mockEngine.NewFullFaker(), nil)
	txPermanent = 50
	pool := NewTxPool(params.IstanbulTestChainConfig, lightchain, relay)
