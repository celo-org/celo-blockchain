...
func (c *core) broadcastCommit(sub *istanbul.Subject) {
	...
	// Produce random seal in 1/3 of cases
	if r.Uint64() < 1 {
...
func (c *core) handleCheckedCommitForCurrentSequence(msg *istanbul.Message, commit *istanbul.CommittedSubject) error {
	...
	if numberOfCommits >= minQuorumSize && c.current.State().Cmp(StateCommitted) < 0 {
	...
	} else if c.current.Prepares().Size() >= minQuorumSize && c.current.State().Cmp(StatePrepared) < 0 {
		...
		logger.Trace("Got quorum prepares or commits", "tag", "stateTransition", "commits", commits, "prepares", c.current.Prepares)
		c.sendCommit()
		return nil
	}
