...
func newBlockChainWithKeys(genesis *core.Genesis, nodeKeys []*ecdsa.PrivateKey) (*core.BlockChain, *Backend) {
	...
	signerBLSMessageFn := SignBLSMessageFn(nodeKeys[0])

	b, _ := New(config, memDB).(*Backend)
	b.Authorize(address, &publicKey, decryptFn, signerFn, signerBLSFn, signerBLSMessageFn)
...
func SignBLSFn(key *ecdsa.PrivateKey) istanbul.BLSSignerFn {
	...
	return func(_ accounts.Account, data []byte) (blscrypto.SerializedSignature, error) {
...
func SignBLSMessageFn(key *ecdsa.PrivateKey) istanbul.BLSMessageSignerFn {
	if key == nil {
		key, _ = generatePrivateKey()
	}

	return func(_ accounts.Account, data []byte, extraData []byte) (blscrypto.SerializedSignature, error) {
		privateKeyBytes, err := blscrypto.ECDSAToBLS(key)
		if err != nil {
			return blscrypto.SerializedSignature{}, err
		}
...
func signBlock(keys []*ecdsa.PrivateKey, block *types.Block) types.IstanbulAggregatedSeal {
	...
	for i, key := range keys {
		...
		sig, err := signFn(accounts.Account{}, msg)
		if err != nil {
...
func newBackend() (b *Backend) {
	...
	key, _ := generatePrivateKey()
	b.Authorize(crypto.PubkeyToAddress(key.PublicKey), &key.PublicKey, decryptFn, SignFn(key), SignBLSFn(key), SignBLSMessageFn(key))
	return
}
