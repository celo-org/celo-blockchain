...
func GetGenesisAndKeys(numValidators int, isFullChain bool) (*core.Genesis, []*ecdsa.PrivateKey) {
	// Setup validators
	var nodeKeys = make([]*ecdsa.PrivateKey, numValidators)
	validators := make([]istanbul.ValidatorData, numValidators)
...
func NewBlockChain(n int, isFullChain bool) (*core.BlockChain, *backend.Backend) {
...
func NewBlockChainWithKeys(isProxy bool, proxiedValAddress common.Address, isProxied bool, genesis *core.Genesis, privateKey *ecdsa.PrivateKey) (*core.BlockChain, *backend.Backend, *istanbul.Config) {
	...
	b, _ := backend.New(&config, memDB).(*backend.Backend)
...
func MakeBlock(keys []*ecdsa.PrivateKey, chain *core.BlockChain, engine *backend.Backend, parent *types.Block) (*types.Block, error) {
	...
	go engine.EventMux().Post(istanbul.FinalCommittedEvent{})

	return block, nil
}

func MakeBlockWithoutSeal(chain *core.BlockChain, engine *backend.Backend, parent *types.Block) *types.Block {
...
func NewBackend() (b *backend.Backend) {
