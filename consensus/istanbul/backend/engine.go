...
func (sb *Backend) verifyCommittedSeals(chain consensus.ChainReader, header *types.Header, parents []*types.Header) error {
	...
	err = blscrypto.VerifyAggregatedSignature(publicKeys, proposalSeal, []byte{}, extra.CommittedSeal.Signature, false)
	if err != nil {
...
func writeCommittedSeals(h *types.Header, round *big.Int, bitmap *big.Int, committedSeals []byte) error {
	...
	istanbulExtra.CommittedSeal.Signature = make([]byte, len(committedSeals))
	copy(istanbulExtra.CommittedSeal.Signature, committedSeals)
	istanbulExtra.CommittedSeal.Bitmap = big.NewInt(0)
	istanbulExtra.CommittedSeal.Bitmap.Set(bitmap)
	istanbulExtra.CommittedSeal.Round = big.NewInt(0)
	istanbulExtra.CommittedSeal.Round.Set(round)
