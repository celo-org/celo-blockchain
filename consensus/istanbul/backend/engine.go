...
func (sb *Backend) verifySealsOnProposal(validators istanbul.ValidatorSet, proposalSeal []byte, bitmap *big.Int, signature []byte) error {
	publicKeys := [][]byte{}
	...
	err := blscrypto.VerifyAggregatedSignature(publicKeys, proposalSeal, []byte{}, signature, false)
	if err != nil {
