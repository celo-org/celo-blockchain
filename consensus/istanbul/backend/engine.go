...
func (sb *Backend) verifyCommittedSeals(chain consensus.ChainReader, header *types.Header, parents []*types.Header) error {
	...
	log.Info("verifyCommittedSeals", "header", header)
	proposalSeal := istanbulCore.PrepareCommittedSeal(header.Hash())
	// 1. Get committed seals from current header
	if len(extra.Bitmap) != snap.ValSet.Size() {
		return errBitmapWrongSize
	}
	publicKeys := [][]byte{}
	for i, hasSigned := range extra.Bitmap {
		if hasSigned == 1 {
	...
	sb.logger.Info("stuff1", "extra", extra)
	sb.logger.Info("stuff2", "publicKeys", publicKeys, "proposalSeal", proposalSeal, "extra.CommittedSeal", extra.CommittedSeal)
...
func (sb *Backend) getValSet(header *types.Header, state *state.StateDB) (map[common.Address][]byte, error) {
	...
	if err == core.ErrSmartContractNotDeployed {
		...
		for _, addr := range newValSetAddresses {
			...
			_, err := sb.iEvmH.MakeStaticCall(*validatorsAddress, getValidatorsFuncABI, "getValidator", []interface{}{}, &validator, maxGasForGetValidators, header, state)
			if err != nil {
...
func writeCommittedSeals(h *types.Header, bitmap []byte, committedSeals []byte) error {
	...
	istanbulExtra.Bitmap = make([]byte, len(bitmap))
	copy(istanbulExtra.Bitmap, bitmap)
