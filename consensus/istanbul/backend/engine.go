...
func (sb *Backend) VerifyLightPlumoProofs(lightProofs []types.LightPlumoProof) error {
	...
	for _, lightProof := range lightProofs {
		...
		lastEpochBlock := snark.EpochBlock{
			...
			MaxNonSigners: uint32(lightProof.LastEpoch.MaxNonSigners),
		...
		_, err = sb.snapshot(sb.chain, uint64(lastEpochBlock.Index), common.Hash{}, nil)
		if err != nil {
