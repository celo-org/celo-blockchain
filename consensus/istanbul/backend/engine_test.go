...
func testSealCommittedOtherHash(t *testing.T, numValidators int) {
	...
	engine.Commit(otherBlock, types.IstanbulAggregatedSeal{}, types.IstanbulEpochValidatorSetSeal{}, &istanbul.BlockProcessResult{})
...
func TestSealCommitted(t *testing.T) {
	...
	result := <-results
	if result.SealedBlock.Hash() != expectedBlock.Hash() {
		t.Errorf("hash mismatch: have %v, want %v", result.SealedBlock.Hash(), expectedBlock.Hash())
	}
}
