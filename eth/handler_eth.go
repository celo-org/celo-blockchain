...
func (h *ethHandler) Handle(peer *eth.Peer, packet eth.Packet) error {
	...
	case *eth.BlockBodiesPacket:
		blockHashes, transactions, epochSnarkData := packet.Unpack()
		return h.handleBodies(peer, blockHashes, transactions, epochSnarkData)
...
func (h *ethHandler) handleBodies(peer *eth.Peer, blockHashes []common.Hash, transactions [][]*types.Transaction, epochSnarkData []*types.EpochSnarkData) error {
	// Filter out any explicitly requested bodies, deliver the rest to the downloader
	filter := len(blockHashes) > 0 || len(transactions) > 0 || len(epochSnarkData) > 0
	if filter {
		blockHashes, transactions, epochSnarkData = h.blockFetcher.FilterBodies(peer.ID(), blockHashes, transactions, epochSnarkData, time.Now())
	}
	if len(blockHashes) > 0 || len(transactions) > 0 || len(epochSnarkData) > 0 || !filter {
		err := h.downloader.DeliverBodies(peer.ID(), transactions, epochSnarkData)
		if err != nil {
