...
func (s *Ethereum) Validator() (eb common.Address, err error) {
	s.lock.RLock()
...
func (s *Ethereum) TxFeeRecipient() (eb common.Address, err error) {
	s.lock.RLock()
...
//
// We regard two types of accounts as local miner account: the validator address
...
func (s *Ethereum) StartMining(threads int) error {
	...
	if !s.IsMining() {
		...
		// Configure the local tax fee recipient address
		txFeeRecipient, err := s.TxFeeRecipient()
		if err != nil {
			log.Error("Cannot start mining without tx fee recipient", "err", err)
			return fmt.Errorf("tx fee recipient missing: %v", err)
		}
