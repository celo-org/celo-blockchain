...
func New(stack *node.Node, config *Config) (*Ethereum, error) {
	...
	proofDb, err := stack.OpenDatabase("plumoproofdata", config.DatabaseCache, config.DatabaseHandles, "eth/db/plumoproofdata/")
	if err != nil {
		return nil, err
	}
	...
	if eth.protocolManager, err = NewProtocolManager(chainConfig, checkpoint, config.SyncMode, config.NetworkId, eth.eventMux, eth.txPool, eth.engine, eth.blockchain, chainDb, proofDb, cacheLimit, config.Whitelist, stack.Server(), stack.ProxyServer()); err != nil {
		return nil, err
	}
