...
func makeTransactions(key *ecdsa.PrivateKey, target int) []*types.Transaction {
	...
	for i := 0; i < target; i++ {
		random := rand.Uint32()
		tx := types.NewTransaction(uint64(random), common.Address{0x1, 0x2, 0x3}, big.NewInt(int64(random)), 100, big.NewInt(int64(random)), nil, nil, nil, nil)
...
func TestRejectUnderpriced(t *testing.T) {
	...
	tx := types.NewTransaction(0, common.Address{0x1, 0x2, 0x3}, big.NewInt(int64(100)), 100, big.NewInt(int64(100)), nil, nil, nil, nil)
