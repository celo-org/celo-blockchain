...
func makeChain(n int, seed byte, parent *types.Block) ([]common.Hash, map[common.Hash]*types.Block) {
	blocks, _ := core.GenerateChain(params.DefaultChainConfig, parent, mockEngine.NewFaker(), testdb, n, func(i int, block *core.BlockGen) {
		block.SetCoinbase(common.Address{seed})

		// If the block number is multiple of 3, send a bonus transaction to the miner
		if parent == genesis && i%3 == 0 {
			signer := types.MakeSigner(params.DefaultChainConfig, block.Number())
...
func (f *fetcherTester) makeBodyFetcher(peer string, blocks map[common.Hash]*types.Block, drift time.Duration) bodyRequesterFn {
	...
	return func(hashes []common.Hash) error {
		...
		headerHashes := make([]common.Hash, 0, len(hashes))
		...
		// Return on a new thread
		go f.fetcher.FilterBodies(peer, headerHashes, transactions, randomness, epochSnarkData, time.Now().Add(drift))
