...
func (f *BlockFetcher) FilterBodies(peer string, blockHashes []common.Hash, transactions [][]*types.Transaction, randomness []*types.Randomness, time time.Time) ([]common.Hash, [][]*types.Transaction, []*types.Randomness) {
	...
	select {
	case filter <- &bodyFilterTask{peer: peer, blockHashes: blockHashes, transactions: transactions, randomness: randomness, time: time}:
	case <-f.quit:
		return nil, nil, nil
	}
	...
	select {
	case task := <-filter:
		return task.blockHashes, task.transactions, task.randomness
	case <-f.quit:
		return nil, nil, nil
	}
}
	...
	for {
		...
		case filter := <-f.bodyFilter:
			...
			if len(f.completing) > 0 {
				for i := 0; i < len(task.blockHashes) && i < len(task.transactions) && i < len(task.randomness); i++ {
					...
					for hash, announce := range f.completing {
						...
						matched = true
						if f.getBlock(hash) == nil {
							block := types.NewBlockWithHeader(announce.header).WithBody(task.transactions[i], task.randomness[i])
