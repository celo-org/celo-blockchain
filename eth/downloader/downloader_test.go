...
func (dlp *downloadTesterPeer) RequestBodies(hashes []common.Hash) error {
	txs, uncles, randomness, parentSeals := dlp.chain.bodies(hashes)
	go dlp.dl.downloader.DeliverBodies(dlp.id, txs, uncles, randomness, parentSeals)
	return nil
}
...
func TestInactiveDownloader62(t *testing.T) {
	...
	if err := tester.downloader.DeliverBodies("bad peer", [][]*types.Transaction{}, [][]*types.Header{}, []*types.Randomness{}, []*types.BlockSeal{}); err != errNoSyncActive {
		t.Errorf("error mismatch: have %v, want  %v", err, errNoSyncActive)
	}
}
