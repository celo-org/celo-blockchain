...
func (d *Downloader) fetchBodies(from uint64) error {
	...
	var (
		deliver = func(packet dataPack) (int, error) {
			...
			return d.queue.DeliverBodies(pack.peerID, pack.transactions, pack.randomness)
		}
...
func (d *Downloader) importBlockResults(results []*fetchResult) error {
	...
	for i, result := range results {
		blocks[i] = types.NewBlockWithHeader(result.Header).WithBody(result.Transactions, result.Randomness)
	}
...
func (d *Downloader) commitPivotBlock(result *fetchResult) error {
	block := types.NewBlockWithHeader(result.Header).WithBody(result.Transactions, result.Randomness)
...
func (d *Downloader) DeliverBodies(id string, transactions [][]*types.Transaction, randomness []*types.Randomness) (err error) {
	return d.deliver(d.bodyCh, &bodyPack{id, transactions, randomness}, bodyInMeter, bodyDropMeter)
}
