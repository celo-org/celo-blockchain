...
var (
	...
	truffleFlag = cli.StringFlag{
		Name:  "truffle",
		Usage: "Path to the json file generated by Truffle",
	}
...
func abigen(c *cli.Context) error {
	utils.CheckExclusive(c, abiFlag, jsonFlag, solFlag, vyFlag, truffleFlag) // Only one source can be selected.
	...
	if c.GlobalString(abiFlag.Name) != "" || c.GlobalString(truffleFlag.Name) != "" {
		...
		if c.GlobalString(abiFlag.Name) != "" {
			...
			// truffle file
			input := c.GlobalString(truffleFlag.Name)
			var fileData []byte
			if input == "-" {
			...
			if err != nil {
				utils.Fatalf("Failed to read input truffle file: %v", err)
			}

			var jsonValue map[string]interface{}
			err = json.Unmarshal(fileData, &jsonValue)
			if err != nil {
				utils.Fatalf("Failed decode json: %v", err)
			}
			abi, err = json.Marshal(jsonValue["abi"])
			if err != nil {
				utils.Fatalf("Can't encode truffle[abi] json: %v", err)
			}
			abis = append(abis, string(abi))
		}
